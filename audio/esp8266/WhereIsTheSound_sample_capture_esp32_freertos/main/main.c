...
#define FRAME_SIZE 512	  // # samples (1024)
#define SAMPLING_RATE 16  // kHz (32)
			...
			for (int i = 0; i < ret_num; i += ADC_RESULT_BYTE) {
				...
				if (ADC_CONV_MODE == ADC_CONV_BOTH_UNIT || ADC_CONV_MODE == ADC_CONV_ALTER_UNIT) {
					if (check_valid_data(p)) {
						...
						if (ch_1_frame_pointer >= FRAME_SIZE) {
							ch_1_frame_pointer = 0;
							ch_1_send = 1;
						}
						if (ch_2_frame_pointer >= FRAME_SIZE) {
							ch_2_frame_pointer = 0;
							ch_2_send = 1;
						}
