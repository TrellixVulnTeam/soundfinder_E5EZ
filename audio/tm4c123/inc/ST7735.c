...
void ST7735_XYplotInit(char *title, int32_t minX, int32_t maxX, int32_t minY, int32_t maxY){
  ...
  ST7735_FillRect(0, 32, 128, 128, ST7735_Color565(228,228,228)); // light grey graph area
	
	ST7735_SetCursor(1,1);
	ST7735_OutString(title);	// set title
...
void ST7735_XYplot(uint32_t num, int32_t bufX[], int32_t bufY[]){
  ...
  for(int i = 0; i < num; i++){
		// scaling code
		int32_t tempX = (127 * (bufX[i] - plotMinX))/(plotMaxX - plotMinX);
		int32_t tempY = (127 * (plotMaxY-bufY[i]))/(plotMaxY - plotMinY);
		ST7735_DrawPixel(tempX, tempY + 32, ST7735_BLUE);
	}
}
